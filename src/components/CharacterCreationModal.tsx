...
import { getAvailableFates, getStartRegionsForRace, resolveFateStartLocationTag } from '../data/world/fateData';
...
export function CharacterCreationModal({isOpen,onComplete,onCancel}:CharacterCreationModalProps){
  ...
  const finish=()=>{
    ...
    state={...state,rupees:fate.startingRupees,inventory,storyFlags,fate:createInitialFateState(fate,regionId,startHex.id,state.dayCount,state.dialogueCount),worldMap:createInitialWorldMapState(startLocationTag,storyFlags)};
    onComplete(state);
  };
