...
import { DUNGEON_REWARD_ITEM_DATABASE, getDungeonCoreItemId, getDungeonRelicItemId } from '../data/items/dungeonRewardItems';
import { getAdultMinPhysicalAge, isAdultPhysicalAge } from '../config/agePolicy';
...
export function DungeonExplorerModal({isOpen,dungeonId,playerState,onClose,onUpdatePlayer,onLog}:Props){
  ...
  const interact=(tile:any)=>{
    ...
    if((tile.kind==='TRAP'||tile.kind==='ADULT_TRAP')&&!state.triggeredTrapIds.includes(tile.id)){
      ...
      if(tile.kind==='TRAP'){const trap=NORMAL_DUNGEON_TRAPS.find(t=>t.id===tile.trapId)||NORMAL_DUNGEON_TRAPS[0];base={...base,hp:Math.max(1,base.hp-Math.round(base.maxHp*(trap.hpRatioDamage||0))),sanity:Math.max(1,base.sanity-(trap.sanityDamage||0))};if(trap.timeMinutes)base=advanceGameTime(base,trap.timeMinutes);log(`${trap.name}: ${trap.description}`);}else{const adult=ADULT_DUNGEON_TRAP_SLOTS.find(t=>t.id===tile.adultTrapSlotId);if(!isAdultPhysicalAge(base.profile?.physicalAge)){log('연령 제한이 걸린 특수 함정 슬롯이 비활성화되었다.');}else if(adult&&(adult.name||adult.sceneReference||adult.effectReference)){base={...base,activeDungeonAdultTrapSlotId:adult.id,activeDungeonAdultTrapExpiresAtDialogue:Number(base.dialogueCount||0)+1};log(`${adult.name||'성인 함정'}이 발동했다. 다음 장면에서 사용자 작성 참조 슬롯을 사용한다.`);}else{log('사용자 작성용 성인 함정 슬롯이다. 아직 내용이 비어 있어 일반 탐사 처리로 넘어간다.');}}
    }
    if(tile.kind==='TREASURE'&&!state.claimedTreasureIds.includes(tile.id)){next.claimedTreasureIds=Array.from(new Set([...next.claimedTreasureIds,tile.id]));const rupees=35+dungeon.rewardTier*45;const itemId=getDungeonRelicItemId(dungeon.regionId);const def=DUNGEON_REWARD_ITEM_DATABASE[itemId];const quantity=1+dungeon.rewardTier;const item={id:itemId,name:def.name,quantity,category:def.category,description:def.description};const inv=base.inventory.map(i=>({...i}));const found=inv.find(i=>i.id===item.id);if(found)found.quantity+=item.quantity;else inv.push(item);base={...base,rupees:base.rupees+rupees,inventory:inv};base=dispatchGameEvent(base,'ITEM_GAINED',{itemId,itemName:def.name,quantity}).nextState;log(`보물 발견: ${item.name} x${item.quantity}, ${rupees} 루피.`);}
    if(tile.kind==='LOOT_ROOM'){
      if(!state.bossDefeated){log('출구의 전리품방은 보스를 쓰러뜨리기 전에는 열리지 않는다.');return;}if(!state.claimedTreasureIds.includes(tile.id)){const qty=2+dungeon.rewardTier*2,rupees=120*dungeon.rewardTier;const itemId=getDungeonCoreItemId(dungeon.id);const def=DUNGEON_REWARD_ITEM_DATABASE[itemId];const item={id:itemId,name:def.name,quantity:qty,category:def.category,description:def.description};const inv=base.inventory.map(i=>({...i}));const found=inv.find(i=>i.id===item.id);if(found)found.quantity+=qty;else inv.push(item);base={...base,rupees:base.rupees+rupees,inventory:inv};base=dispatchGameEvent(base,'ITEM_GAINED',{itemId,itemName:def.name,quantity:qty}).nextState;next.claimedTreasureIds=Array.from(new Set([...next.claimedTreasureIds,tile.id]));next.completed=true;log(`던전 공략 완료. 전리품방에서 ${item.name} x${qty}, ${rupees} 루피를 획득했다.`);}}
