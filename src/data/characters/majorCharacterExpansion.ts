...
export function canRecruitMajorCharacter(state:PlayerState,id:string){const c=state.majorCharacters?.[id];if(!c||!c.isAlive||c.isRecruited||!c.isRecruitable)return false;const hasMet=Boolean(c.hasMet||(c.interactionHistory?.length||0)>0);if(!hasMet||!c.currentHexId||c.currentHexId!==state.worldMap?.currentHexId)return false;return c.trust>=(c.recruitmentTrust??55);}
export function createCompanionFromMajorCharacter(c:MajorCharacter,playerLevel=1):CompanionData{
  ...
  return{id:c.companionId||`companion_${c.id}`,name:c.name,gender:'남성',physicalAge:20,race:c.race,beastkinType:c.beastkinType,appearance:`${c.title}. ${c.personality}`,level,experience:0,hp,maxHp:hp,mp,maxMp:mp,sanity,maxSanity:sanity,baseStats:stats,stats:{...stats},combatClass:cls,talentPoints:0,learnedTalents:{},learnedSkills:['basic_attack','defend_stance','first_aid'],professions:createBlankProfessions(),equipment:createBlankEquipment(),equipmentEnhancements:{},equippedBagId:null,kind:'HUMANOID',bond:{bondLevel:1,bondExp:0,trust:c.trust,affection:Math.max(0,Math.min(100,c.relationship)),personalFlags:{}},needs:createInitialCompanionNeeds(),combatTactic:'BALANCED',manualCombatControl:false,isActivePartyMember:false};
}
export function recruitMajorCharacter(state:PlayerState,id:string){const c=state.majorCharacters?.[id];if(!c)return{ok:false,state,message:'인물을 찾을 수 없습니다.'};const hasMet=Boolean(c.hasMet||(c.interactionHistory?.length||0)>0);if(!hasMet)return{ok:false,state,message:`${c.name}와(과) 아직 실제로 조우하지 않았습니다.`};if(!c.currentHexId||c.currentHexId!==state.worldMap?.currentHexId)return{ok:false,state,message:`${c.name}와(과) 현재 같은 장소에 있지 않아 영입할 수 없습니다.`};if(c.trust<(c.recruitmentTrust??55))return{ok:false,state,message:`영입에는 신뢰도 ${c.recruitmentTrust??55} 이상이 필요합니다.`};if(!canRecruitMajorCharacter(state,id))return{ok:false,state,message:'현재 이 인물을 영입할 수 없습니다.'};if(state.companions.some(x=>x.id===(c.companionId||`companion_${c.id}`)))return{ok:false,state,message:'이미 동료입니다.'};const companion=createCompanionFromMajorCharacter(c,state.level);return{ok:true,state:{...state,companions:[...state.companions,companion],majorCharacters:{...state.majorCharacters,[id]:{...c,isRecruited:true}}},message:`${c.name}이(가) 동료로 합류했습니다.`};}
