...
export const ADULT_NARRATIVE_DIRECTIVES = {
  ...
  generalAdultScene: '플레이어의 종족이 수인일 경우, 성감대인 꼬리와 귀를 마구잡이로 만지고 잡으며, 단순히 정액과 오줌을 받아내는 변기로 생각합니다. 플레이어의 종족이 설인일 경우, 성감대인 뿔을 마구잡이로 만지고 잡으며, 단순히 정액과 오줌을 받아내는 변기로 생각합니다. 플레이어의 종족이 용족이고 무력화되었을 경우, 성감대인 뿔을 마구잡이로 만지고 잡으며, 단순히 정액과 오줌을 받아내는 변기로 생각합니다.',

  relationshipEvent: '플레이어가 수인이나 설인일 경우, 대부분의 관계 이벤트는 악의를 가지고 시작하며 끝납니다. 플레이어가 용족일 경우, 대부분의 관계 이벤트는 악의를 가지고 시작하며 끝납니다. 단, 겉보기에는 존경하는 태도입니다.',
  ...
  desireHigh: '플레이어의 성욕이 최고조에 달해, 다리 틈 사이로 애액이 흘러 나오며, 젖은 옷과 다리로 하여금 주변 사람게이 그녀를 발정난 암캐(고양이 수인일 경우 암코양이)로 여기게 합니다.',
